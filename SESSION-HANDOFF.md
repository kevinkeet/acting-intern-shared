...
## SCENARIO-BRIEF REMOVAL — DONE (all 14 APs)
**Decision:** the per-timepoint `scenarioBrief` pre-digests the case (and in places leaks the answer — e.g. PAT007 AP3 stated "drug fever and drug-associated neurotoxicity"; PAT005 AP2 stated the FFP conclusion). Piecing the picture together from the chart IS part of what we want to measure. So each `scenarioBrief` is replaced with a **minimal orientation stub** — role + patient + moment in time only, no clinical content.
**Rule:** before stripping a scenario, verify every trigger fact it carried is discoverable in the chart **at or before that AP's anchorDate** (notes/labs/imaging/problems). Questions stay verbatim (validated).
**Caveat to record in methods:** the source docx presented these as vignettes (aliquot text). Removing the brief is a deliberate departure from the validated *administration* format (scoring is unchanged). Consider A/B testing (with vs. without) once API credits are back.
